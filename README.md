...
- **Sandbox 边界**：`ctx.sandbox` 主要约束测试进程的文件效果，不等于网络、凭据或全系统隔离；Windows 或其他后端可能报告 `partial`。`confined.enforcement` 不是 `full`（包括 `partial`）时默认 fail-closed，不执行测试命令，也不裸回退到原始 argv。`testCmd` 仍是任意 shell 命令，只应在受控任务和受信任的 Harness profile 中使用。
